...
difficulty = 0
tsugi = 0
destroyed_blocks = 0  # 파괴된 블록 수 추가
...
def game_main():
    ...
    if index == 0:  # 타이틀 로고
        ...
        if difficulty > 0:
            ...
            mouse_c = 0
            score = 0
            destroyed_blocks = 0  # 게임 시작 시 파괴된 블록 수 초기화
            tsugi = 0
            cursor_x = 0
            cursor_y = 0
            set_neko()
    ...
    draw_txt("DESTROYED " + str(destroyed_blocks), 160, 100, 24, "green", "INFO")  # 파괴된 블록 수 표시
    if tsugi > 0:
...
bg = tkinter.PhotoImage(file="new_bg.png")  # 새로운 배경 이미지로 변경
